...
| DEC-021 | Reddit operating-risk acceptance | Use one server-side Reddit application credential and app-level read token for discovery. Users do not connect Reddit to Mentionish. Poll conservatively with global query budgets and jitter, obey returned rate-limit/retry headers, cache/deduplicate globally, and stop promptly on credential or authorization failure. The owner accepts that app-level versus per-user OAuth does not change the underlying commercial-policy/enforcement risk and does not require a separate commercial agreement as a launch gate. Record this decision as `REDDIT_POLICY_RISK_ACCEPTED`; no configuration or UI may imply Reddit approval. Never scrape, evade limits, rotate credentials to bypass enforcement, use Reddit write APIs, or auto-post. Keep discovery kill-switchable and preserve HN as a secondary fallback. |
| DEC-022 | Dashboard sign-in | Use Supabase Google OAuth as the primary v1 sign-in method, with email magic links as fallback. Require a provider-verified or Supabase-verified email before the trial starts. Password auth and user-facing Reddit OAuth are out of scope. The Chrome extension relies on the user’s existing Reddit browser session only for user-initiated DOM insertion; it never reads or transmits Reddit credentials. |
...
| DEC-024 | Frontend design system | `docs/theme.css` is the canonical theme and design-token source for every Mentionish frontend surface. Dashboard styles import it directly and consume semantic variables rather than defining a separate palette. New colors, typography, spacing, radii, shadows, focus states, charts, and sidebar treatments must be represented in the canonical theme before use. Hard-coded product colors are prohibited outside that file; third-party brand marks are the only narrow exception. Light and `.dark` modes must both remain token-driven. |
| DEC-025 | Temporary Reddit cookie fallback | Until official Reddit application credentials are available, the owner authorizes a separately gated, accepted-risk `rdt-cli` cookie-backed read transport using one dedicated account. This temporary decision supersedes the no-scraping portion of DEC-021 only for the documented read-only fallback; OAuth remains preferred. Require `REDDIT_COOKIE_BACKEND_RISK_ACCEPTED`, conservative global budgets, sequential execution, cache/deduplication, auth halt, rate-limit cooldown, deletion cleanup, a kill switch, and HN continuity. Never expose the cookie, rotate identities to evade enforcement, invoke Reddit write commands, or claim Reddit approval. Follow `reddit-cookie-fallback.md`. |
| DEC-026 | Verified Reddit desktop fallback | The live `rdt-cli` test returned `403 forbidden` and browser-cookie extraction returned no credentials. Agent Reach now prioritizes OpenCLI on desktops. Use the separately gated, read-only OpenCLI browser-session transport for supervised local validation only, following `reddit-opencli-fallback.md`. Preserve all DEC-025 budgets and shutdown controls, block every write command, and keep official OAuth as the production migration target. |
...
- DEC-C14: Supabase Google OAuth is the primary v1 dashboard authentication method; email magic links remain available as fallback.
...
- DEC-C16: Every frontend surface uses `docs/theme.css` as its canonical semantic-token source; parallel palettes are not allowed.
